...
requestHeaders={
    ...
    'Cookie': 'ys-userName=s%3A36072107001; JSESSIONID=abc_PzJALx8tFdYzHwmQv',
    'Pragma': 'no-cache',
...
page_size=500
# 写入文件名称
writepath=r"/Users/leo/trscases/gitproject/StockSpider/medicalInfomation.xlsx" 
#写表头
wb=Workbook()  
...
# 定义保存数据的方法
def saveContent(pageVolumn=100,startPage=0,pageSize=15):
    currentRow=2+startPage*pageSize
    for i in range(startPage,startPage+pageVolumn):
        # 重新打开excel
        if(i%10==0):
            wb.save(writepath)
            wb=load_workbook(writepath)   
            sheet=wb.get_sheet_by_name("医保数据")  
        content=getContent(i*pageSize,pageSize)
        for o in content:
            startColum=1
